...
// toggleDarkMode y showToast movidos a utils.js
        ...
        async function handleSearch() {
            ...
            try {
                ...
                <div class="group rounded-2xl border border-slate-100 bg-white p-6 transition-all hover:shadow-xl hover:-translate-y-1 dark:border-slate-800 dark:bg-slate-800/50 cursor-pointer" onclick="viewProfile('${t.id}')">
                    ...
                    <h3 class="mb-1 text-base font-bold">${t.name}</h3>
                    <p class="text-xs font-medium text-slate-500 uppercase tracking-wide mb-4">${t.title ? t.title.split('•')[0].trim() : 'Tutor'}</p>
        ...
        async function loadTopTutors() {
            try {
                ...
                <div class="group relative rounded-2xl border border-slate-100 bg-white p-6 transition-all hover:shadow-xl hover:-translate-y-1 dark:border-slate-800 dark:bg-slate-800/50 cursor-pointer" onclick="viewProfile('${t.id}')">
                    <div class="mb-4 flex items-start justify-between">
                        <div class="relative">
                    ...
                    <h3 class="mb-1 text-lg font-bold text-slate-900 dark:text-white">${t.name}</h3>
                    <p class="mb-4 text-xs font-medium text-slate-500 uppercase tracking-wide">${t.title ? t.title.split('•')[0].trim() : 'Tutor'}</p>
                    <div class="mb-6 flex flex-wrap gap-2">
                        ${(t.badges || []).map(b => `<span class="rounded-full bg-slate-100 px-3 py-1 text-[10px] font-bold text-slate-600 dark:bg-slate-700 dark:text-slate-300">${b.toUpperCase()}</span>`).join('')}
                    </div>
